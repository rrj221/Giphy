...
//save celebrities to local storage initally (put inside somewhere else later)
...
function addButtons() {
	//need if statement here based on return value
...
function deleteButton (celebName, celebIndex) {
	var celebs = JSON.parse(localStorage.getItem('celebsLocal'));
...
$('#celebButtonsEl').on('click', '.checkbox', function() {
	deleteButton($(this).data('celebname'), $(this).data('celebindex'));
});
